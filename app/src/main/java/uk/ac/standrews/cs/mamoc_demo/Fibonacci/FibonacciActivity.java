...
import uk.ac.standrews.cs.mamoc_demo.R;

import uk.ac.standrews.cs.mamoc_client.Execution.ExecutionLocation;
import uk.ac.standrews.cs.mamoc_client.MamocFramework;


public class FibonacciActivity extends DemoBaseActivity {

    private final String task_name = Fibonacci.class.getName();
    ...
    private void runLocal(int N) {
        mamocFramework.execute(ExecutionLocation.LOCAL, task_name, null, N);
    }

    private void runEdge(int N) {
        try{
            mamocFramework.execute(ExecutionLocation.EDGE, task_name, null, N);
        } catch (Exception e){
    ...
    private void runCloud(int N) {
        try{
            mamocFramework.execute(ExecutionLocation.PUBLIC_CLOUD, task_name, null, N);
        } catch (Exception e){
    ...
    private void runDynamically(int N) {
        try{
            mamocFramework.execute(ExecutionLocation.DYNAMIC, task_name, null, N);
        } catch (Exception e){
